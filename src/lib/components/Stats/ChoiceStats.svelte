...
<ul class="flex flex-col gap-8 p-0">
  {#each choices as choice, index (choice.id)}
    ...
    <li animate:flip={{ duration: 200 }} class="relative flex flex-col">
      <p
        class="text-sm m-0 mb-1 text-neutral-700 dark:text-neutral-200 {index === 0
          ? 'font-extrabold'
          : ''}"
      >
        {choice.text}
      ...
      <div
        class="relative flex items-center gap-2 py-2 px-4 text-neutral-100 bg-neutral-200 dark:bg-neutral-900 rounded-md"
      >
        <p class="font-extrabold w-max text-xl z-10">
          {stat}
        </p>
        <p class="text-neutral-300 z-10">
        ...
        <!-- Bar -->
        <div
          class="absolute left-0 top-0 w-full h-full  flex flex-row items-center rounded-md bg-neutral-600 transition-all"
